/*
Code required only for u8 list (byte-granular string) cryption.
*/
void
karacell_u8_to_u32_list_copy(ULONG u8_idx_min,u8 *u8_list_base,ULONG u32_idx_min,u32 *u32_list_base,u32 u8_count_minus_1){
  u8 shift;
  ULONG u32_idx;
  ULONG u32_idx_post;
  u32 u32_item;
...
void
karacell_u32_to_u8_list_copy(ULONG u8_idx_min,u8 *u8_list_base,ULONG u32_idx_min,u32 *u32_list_base,u32 u8_count_minus_1){
  ULONG u32_idx;
  ULONG u32_idx_post;
  u32 u32_item;
  ULONG u8_count;
...
u8
karacell_u8_list_crypt(u32 entropy_list_base[KARACELL_MASTER_KEY_U32_COUNT_MAX],u64 file_idx,karacell_t *karacell_base,u8 *hash_type_base,ULONG *u8_count_base,ULONG u8_idx_min,u8 *u8_list_base,u8 *unauthenticated_status_base){
...
/*
This code is equivalent to karacell_u32_list_crypt() with adaptations for converting u8 to u32.
*/
  ...
  do{
    ...
    if(decrypt_status){
      ...
      karacell_u8_to_u32_list_copy(u8_idx_min,u8_list_base,0,header_u32_list_base,KARACELL_HEADER_SIZE-1);
    ...
    if(payload_size){
      ...
      while(payload_block_idx<payload_block_idx_max){
        payload_block_idx++;
        karacell_u8_to_u32_list_copy(u8_idx_min,u8_list_base,0,block_base,KARACELL_BLOCK_SIZE-1);
    ...
    if(tail_u8_count){
      ...
      karacell_u8_to_u32_list_copy(u8_idx_min,u8_list_base,0,block_base,tail_u8_count_minus_1);
    }
    karacell_tail_crypt(decrypt_status,karacell_base,payload_block_idx_max,(u32)(payload_size),0,block_base);
    if(tail_u8_count){
      karacell_u32_to_u8_list_copy(u8_idx_min,u8_list_base,0,block_base,tail_u8_count_minus_1);
    }
    ...
    if(decrypt_status){
      ...
      *unauthenticated_status_base=unauthenticated_status;
      if(!unauthenticated_status){
        ...
        karacell_u8_to_u32_list_copy(u8_idx_min,u8_list_base,0,hash_xor_all_base,hash_u8_count_minus_1);
